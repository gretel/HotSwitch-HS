...
HotkeyController.new = function(mainController)
    ...
    obj.returnAction = function(self)
        ...
        local returnWindows = self.keyStatusModel.sortedWindows or self.windowModel:getCachedOrderedWindowsOrFetch()
        self.windowModel:focusWindow(returnWindows[
    ...
    obj.deleteAction = function(self)
        self.isRegistrationMode = false
        local deleteWindows = self.keyStatusModel.sortedWindows or self.windowModel:getCachedOrderedWindowsOrFetch()
        local window = deleteWindows[self.panelLayoutView:getSelectedRowPosition()]
    ...
    obj.doKeyAction = function(self, key)
        ...
        if self.isRegistrationMode then
            ...
            -- Find ALL windows matching this key (supports cycling for multi-window apps)
            local matchingKeyStatuses = {}
            for j = 1, #self.keyStatusModel.registeredAndAutoGeneratedKeyStatuses do
                local ks = self.keyStatusModel.registeredAndAutoGeneratedKeyStatuses[j]
                if ks.key == key then
                    table.insert(matchingKeyStatuses, ks)
                end
            end

            if #matchingKeyStatuses == 0 then
                return
            end

            -- Single window: focus and close panel (original behavior)
            if #matchingKeyStatuses == 1 then
                self:finish()
                self.windowModel:focusWindow(matchingKeyStatuses[1].window)
                return
            end

            -- Multiple windows with same key: cycle to the next one, keep panel open
            local idx = self.cycleKeyIndexes[key] or 0
            self.cycleKeyIndexes[key] = (idx % #matchingKeyStatuses) + 1
            local target = matchingKeyStatuses[self.cycleKeyIndexes[key]]

            -- Don't close panel on focus — unwatch so Hammerspoon deactivation doesn't trigger finish()
            self.appWatchModel:unwatchAppliationDeactivated()
            self.windowModel:focusWindow(target.window)

            -- Move the panel's selection highlight to match the focused window
            local cycleWindows = self.keyStatusModel.sortedWindows or self.windowModel:getCachedOrderedWindowsOrFetch()
            for pos, win in ipairs(cycleWindows) do
                if win:id() == target.windowId then
                    self.panelLayoutView.selectedRowCanvasView.position = pos
                    self.panelLayoutView.selectedRowCanvasView:replaceSelectedRow()
                    break
                end
            end
        end
